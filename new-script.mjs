...
// ======================== TUN 模式 ========================
  ...
  // 自托管规则集（GitHub: guoyingwei6/clash-rules-academic）
  // academic=学术直连, ai-extra=AI补充, google-extra=谷歌补充, node-select=个性化杂项
  const selfHostedRpList = [
    { name: "academic",     file: "Academic.yaml" },
    { name: "ai-extra",     file: "AI-Extra.yaml" },
    { name: "google-extra", file: "Google-Extra.yaml" },
    { name: "node-select",  file: "Node-Select.yaml" },
  ];
  selfHostedRpList.forEach(r => {
    config["rule-providers"][r.name] = {
      ...rpBase,
      behavior: "classical",
      url: `https://raw.githubusercontent.com/guoyingwei6/clash-rules-academic/main/Rules/${r.file}`,
      path: `./ruleset/guoyingwei6/${r.file}`
    };
  });
  ...
  config.rules.push(
    // 自托管规则集（优先级高于上游大杂烩规则）
    "RULE-SET,academic,全局直连",
    "RULE-SET,ai-extra,AI",
    "RULE-SET,google-extra,谷歌服务",
    "RULE-SET,node-select,节点选择",
    // AI
    "RULE-SET,AI,AI",
